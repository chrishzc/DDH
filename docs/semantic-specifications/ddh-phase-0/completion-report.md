...
- Specification: `DDH-P0-SPEC-001@1.1.0`
...
Version `1.1.0` replaces the earlier product-example projection with
DDH-native workspace、path、manifest、runner and Verification Asset scenarios.
The sanitized `1.0.0` historical package structure remains under
`docs/semantic-specifications/archive/ddh-phase-0/1.0.0/`.
...
## Version 1.1.0 Projection Checks

- Current package identity and every Task Specification reference resolve to
  `DDH-P0-SPEC-001@1.1.0`.
- Contract Envelope protocol identity remains independently pinned to
  `1.0.0`.
- The current package contains only DDH-native developer-tool scenario
  vocabulary; the retired generic target-domain projection is absent.
- Developer-tool scenarios preserve the original success、rejection、
  concurrency、recovery、scope-impact and layered-completion coverage.
- The archived `1.0.0` structure contains 34 files. A later explicit cleanup
  directive sanitized residual example vocabulary, so it is no longer a
  byte-exact copy of the pre-amendment package.

## Original Contract Completion Review

For the original contract structure, an independent read-only Critic first
rejected the package on five blockers.
